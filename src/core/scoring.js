...
export function computeMetrics({ mr, so, sm, sessionStart }) {
  ...
  const smT = sm.trials
  const totalPos = smT.reduce((s, t) => s + t.totalTargets, 0) || 1
  const sm_positionRecallRate = smT.reduce((s, t) => s + t.correctPositions, 0) / totalPos
  const sm_typeRecallRate = smT.reduce((s, t) => s + t.correctTypes, 0) / totalPos
  const sm_capacity = smT.length ? mean(smT.map(t => t.correctPositions)) : 0

  const totalItems = real.length + soT.length + smT.length
  const overallAccuracy = totalItems
    ? (real.filter(t => t.correct).length + soT.filter(t => t.correct).length + smT.reduce((s, t) => s + t.correctPositions, 0))
    / (real.length + soT.length + (smT.reduce((s, t) => s + t.totalTargets, 0) || 1))
    : 0
  ...
  return {
    ...
    sm_positionRecallRate, sm_typeRecallRate, sm_capacity,
